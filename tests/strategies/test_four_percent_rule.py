...
class TestFourPercentRule:
    ...
    def test_four_percent_rule_with_inflation(self):
        ...
        strategy = FourPercentRule()
        params = PortfolioParams(init_balance=1_000_000.0, equity_pct=0.6, fees_bps=50)
        state = YearState(year=2024, age=66, balance=950_000.0, inflation=0.03)
    ...
    def test_four_percent_rule_with_zero_inflation(self):
        ...
        strategy = FourPercentRule()
        params = PortfolioParams(init_balance=500_000.0, equity_pct=0.8, fees_bps=25)
        state = YearState(year=2024, age=65, balance=500_000.0, inflation=0.0)
    ...
    def test_four_percent_rule_with_negative_inflation(self):
        ...
        params = PortfolioParams(init_balance=1_000_000.0, equity_pct=0.6, fees_bps=50)
        state = YearState(
            year=2024, age=67, balance=1_100_000.0, inflation=-0.02  # 2% deflation
        )
    ...
    def test_four_percent_rule_with_high_inflation(self):
        ...
        strategy = FourPercentRule()
        params = PortfolioParams(init_balance=2_000_000.0, equity_pct=0.7, fees_bps=75)
        state = YearState(
            year=2024, age=68, balance=1_800_000.0, inflation=0.08  # 8% inflation
        )
    ...
    def test_four_percent_rule_different_initial_balances(self):
        ...
        for init_balance, expected_withdrawal in test_cases:
            params = PortfolioParams(
                init_balance=init_balance, equity_pct=0.6, fees_bps=50
            )
            withdrawal = strategy.calculate_withdrawal(None, params)
    ...
    def test_four_percent_rule_current_balance_ignored(self):
        ...
        state1 = YearState(
            year=2024, age=65, balance=1_200_000.0, inflation=0.02  # Higher balance
        )
        state2 = YearState(
            year=2024, age=65, balance=800_000.0, inflation=0.02  # Lower balance
        )
