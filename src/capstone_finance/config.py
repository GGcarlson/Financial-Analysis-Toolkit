...
class ConfigModel(BaseModel):
    ...
    strategy: str = Field(
        "four_percent_rule", 
        description="Withdrawal strategy to use"
    )
    years: int = Field(
        30, 
        ge=1, 
    ...
    paths: int = Field(
        1000, 
        ge=1, 
        description="Number of simulation paths to run"
    )
    seed: int = Field(
        42, 
    ...
    equity_pct: float = Field(
        ...
        ge=0, 
        le=1, 
        description="Equity allocation (0.0 to 1.0)"
    )
    fees_bps: int = Field(
        50, 
        ge=0, 
        description="Annual fees in basis points"
    )
    
    # Market simulation parameters
    market_mode: str = Field(
        "lognormal",
        description="Market simulation mode (lognormal or bootstrap)"
    )
    ...
    window: int = Field(
        3, 
        ge=1, 
        description="Window size for endowment strategy moving average"
    )
    ...
    guard_pct: float = Field(
        ...
        gt=0, 
        lt=1, 
        description="Guardrail percentage for Guyton-Klinger strategy"
    )
    raise_pct: float = Field(
        0.10, 
        gt=0, 
        lt=1, 
        description="Raise percentage for Guyton-Klinger strategy"
    )
    cut_pct: float = Field(
        0.10, 
        gt=0, 
        lt=1, 
        description="Cut percentage for Guyton-Klinger strategy"
    )
