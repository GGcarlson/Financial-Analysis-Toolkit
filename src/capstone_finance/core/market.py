...
class MarketSimulator:
    ...
    def _generate_bootstrap(self, n_paths: int, n_years: int) -> NDArray[np.float64]:
        ...
        if remainder_years > 0:
            ...
            remainder_indices = self.rng.integers(0, len(all_years), size=(n_paths, remainder_years))
    ...
    def _prepare_bootstrap_data(self) -> None:
        ...
        self._block_returns = np.array([
            returns[i:i+5] for i in range(n_blocks)
        ])


def load_shiller_data(cache_dir: Optional[Path] = None) -> pd.DataFrame:
